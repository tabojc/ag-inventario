...
$.extend($.fn, {
				...
				function handle() {
					...
					if ( validator.settings.submitHandler ) {
						if ( validator.submitButton ) {
							...
							hidden = $("<input type='hidden'/>").prop("name", validator.submitButton.name).val( $(validator.submitButton).val() ).appendTo(validator.currentForm);
						}
	...
	removeAttrs: function( attributes ) {
		...
		$.each(attributes.split(/\s/), function( index, value ) {
			result[value] = $element.prop(value);
...
$.extend($.validator, {
	...
	messages: {
		...
		equalTo: "Ingrese el mismo valor",
                notEqual: "Ingrese un valor distinto",
                letter: "Sólo Letras",
		maxlength: $.validator.format("Favor, ingrese no mas de {0} caracteres"),
		minlength: $.validator.format("Favor, ingrese al menos {0} caracteres"),
		rangelength: $.validator.format("Favor, ingrese un valor entre {0} y {1} largo de caracteres"),
		...
		showLabel: function( element, message ) {
			...
			if ( label.length ) {
				...
				label = $("<" + this.settings.errorElement + ">")
					.prop("for", this.idOrName(element))
